...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK()) nSubsidy = 20000 * COIN;
    else if (nHeight <= 8000) nSubsidy = 6 * COIN;
    else if (nHeight <= 16000) nSubsidy = 8 * COIN;
    else if (nHeight <= 30000) nSubsidy = 10 * COIN;
    else if (nHeight <= 50000) nSubsidy = 12 * COIN;
    else nSubsidy = 14 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ZIO: We use certain sporks during IBD, so check to see if they are
