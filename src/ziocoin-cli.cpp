...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Zio Coin Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  ziocoin-cli [options] <command> [params]  " + _("Send command to Zio Coin Core") + "\n" +
                        "  ziocoin-cli [options] help                " + _("List commands") + "\n" +
                        "  ziocoin-cli [options] help <command>      " + _("Get help for a command") + "\n";
